...
def entrypoint(response, event):
    """
    Gets tweets from a Twitter list and create a list of dicts with them.
    ...
    tweet_list = gt.get_list_tweets(int(event['identifier']), event['aux_data']['data_ini'], event['aux_data']['data_fim'])
    return tweet_list
